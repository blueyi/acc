...
**说明**: 包含 CMake 构建所需的模块文件和交叉编译工具链配置，封装了 LLVM/MLIR 的查找逻辑和 TableGen 代码生成规则。
...
│   └── PassPipeline.h            # LLVM Pass Pipeline 配置
├── CodeGen/
...
**说明**: 公开头文件遵循 LLVM 的目录约定，按功能模块组织。`.td` 文件（TableGen 定义）和 `.h` 文件（C++ 声明）放在同一目录下，方便开发时对照查阅。
...
│   └── PassPipeline.cpp          # LLVM IR 优化 Pipeline 配置
├── CodeGen/                      # 代码生成
│   ├── CMakeLists.txt
...
**说明**: `lib/` 目录包含所有核心功能的 C++ 实现。每个子目录对应一个独立的 CMake 库目标，便于模块化构建和依赖管理。
...
**说明**: 提供用户可直接使用的命令行工具。`ac-opt` 是日常开发中最常用的工具，用于测试和调试 MLIR Pass。
...
test/
├── CMakeLists.txt
├── lit.cfg.py                    # lit 测试框架全局配置
├── lit.site.cfg.py.in            # 构建目录的 lit 站点配置模板
├── Dialect/
│   ├── ACHigh/
│   │   ├── ops.mlir              # 算子定义正确性测试
│   │   ├── types.mlir            # 类型系统测试
